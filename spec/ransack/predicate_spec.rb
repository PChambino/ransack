  ...
  describe Predicate do
    ...
    describe 'present' do
      ...
      it 'generates a condition for boolean false' do
        ...
        field = "#{quote_table_name("people")}.#{quote_column_name("name")}"
        @s.result.to_sql.should match /NOT \(\(#{field} IS NOT NULL AND #{field} != ''\)\)/
      end
    ...
    describe 'blank' do
      ...
      it 'generates a condition for boolean false' do
        @s.name_blank = false
        field = "#{quote_table_name("people")}.#{quote_column_name("name")}"
        @s.result.to_sql.should match /NOT \(\(#{field} IS NULL OR #{field} = ''\)\)/
      end
    ...
    describe 'true' do
      ...
      it 'generates a condition for boolean false' do
        ...
        field = "#{quote_table_name("people")}.#{quote_column_name("awesome")}"
        @s.result.to_sql.should match /NOT \(#{field} = #{
          ActiveRecord::Base.connection.quoted_true}\)/
      end
    ...
    describe 'null' do
      ...
      it 'generates a condition for boolean false' do
        @s.name_null = false
        field = "#{quote_table_name("people")}.#{quote_column_name("name")}"
        @s.result.to_sql.should match /NOT \(#{field} IS NULL\)/
      end
    ...
    describe 'not_null' do
      ...
      it 'generates a condition for boolean false' do
        ...
        field = "#{quote_table_name("people")}.#{quote_column_name("name")}"
        @s.result.to_sql.should match /NOT \(#{field} IS NOT NULL\)/
      end
